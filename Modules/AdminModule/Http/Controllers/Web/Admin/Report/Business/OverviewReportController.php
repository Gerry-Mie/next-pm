...
class OverviewReportController extends Controller
{
    ...
    public function getBusinessOverviewReport(Request $request)
    {
        ...
        foreach ($amounts as $amount) {
            foreach ($earningAmount as $key=> $earning) {
                if ($amount[$group_by_deterministic] == $earning[$group_by_deterministic]) {
                    $all_expenses_and_earnings[$key] = array_merge($amount, $earning);
                }
            }
            ...
            if (!isset($earningAmount[$key]) && !isset($bonus_amounts[$key])) {
                $all_expenses_and_earnings[$key] = $amount;
            }

            if (isset($all_expenses_and_earnings[$key]) && !array_key_exists('earning', $all_expenses_and_earnings[$key])) {
        ...
        if($deterministic == 'month') {
            ...
            foreach ($months as $month) {
                ...
                foreach ($all_expenses_and_earnings as $item) {
                    if ($item['month'] == $month) {
                        $chart_data['earnings'][] = with_decimal_point(data_get($item, 'admin_commission', 0)+ $item['earning']??0);
        ...
        elseif ($deterministic == 'year') {
            foreach ($all_expenses_and_earnings as $item) {
                $chart_data['earnings'][] = with_decimal_point($item['admin_commission'] + $item['earning']??0);
                $chart_data['expenses'][] = with_decimal_point(data_get($item, 'discount_by_admin', 0) + data_get($item, 'coupon_discount_by_admin', 0) + data_get($item, 'campaign_discount_by_admin', 0) + data_get($item, 'bonus', 0));
