...
class BusinessInformationController extends Controller
{
    ...
    public function businessInformationGet(Request $request)
    {
        ...
        return view('businesssettingsmodule::admin.business', compact('dataValues', 'webPage', 'businessLogoFullPath', 'businessFaviconFullPath'));
    }
    ...
    {
        $this->authorize('business_update');
        collect(['provider_can_cancel_booking', 'provider_can_edit_booking', 'provider_self_registration', 'suspend_on_exceed_cash_limit_provider', 'provider_self_delete', 'provider_subscription', 'provider_commision'])->each(fn($item, $key) => $request[$item] = $request->has($item) ? (int)$request[$item] : 0);
